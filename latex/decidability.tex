...
\begin{document}
	...
	\begin{frame}{First-Order Logic}
		...
		\begin{tabular}{p{.5\linewidth}p{.5\linewidth}}
			\visible<2->{
				Formulas consist of:
				\begin{itemize}
					\visible<2->{\item Quantors: $ \forall, \: \exists $}
	...
	\begin{frame}{What Is Decidability?}
		For logic (there is a more general definition):
		\begin{itemize}
	...
	\begin{frame}{Proof of Theorem 1}
		...
		An example for a word in $ \Sigma_{2} $:
		\[
			\begin{pmatrix}
				0\\
				1\\
	...
	\begin{frame}{Proof of Theorem 1}
		\visible<1->{
			...
			\begin{itemize}
				...
				\item $ \varphi_{i} \coloneqq Q_{i+1} x_{i+1} \dots Q_{l} x_{l} \: [\psi(x_{1}, \dots, x_{l})] $ \: and by convention $ \varphi_{l} \coloneqq \psi $
	...
	\begin{frame}{Proof of Theorem 1}
		\begin{itemize}
			\item Calculate one addition automaton for each addition term in $ \varphi_{l} $
			\item Combine them:
				\begin{itemize}
				...
				\end{itemize}
				in a way that they behave like $ \varphi_{l} $, i. e. that the resulting automaton accepts tuples $ (a_{1}, \dots, a_{l}) \in \mathbb{N}^{l} $ for which $ \varphi_{l}(a_{1}, \dots, a_{l}) $ is true.
		\end{itemize}
		\vspace{.3cm}
		
		\visible<2->{We call that automaton $ A_{l} $.}
		\vspace{.3cm}
